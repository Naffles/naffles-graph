...
export function handleUserStaked(event: UserStakedEvent): void {
  ...
  // loop through all the stakeSessions of stakeHisotry and set the unstakedAt to the current block timestamp if unstakedAt is null
  const stakeSessionId = event.params.userAddress.toHex() + "-" + event.params.nftId.toString() + "-" + event.block.timestamp.toString(); 
...
export function handleUserUnstaked(event: UserUnstakedEvent): void {
  ...
  const lastId = stakeHistory.stakeSessionIds[stakeHistory.stakeSessionIds.length - 1];
